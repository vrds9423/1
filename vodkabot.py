...
def NOTIFIED_ADD_CONTACT(op):
    try:
        sendMessage(op.param1, client.getContact(op.param1).displayName + "✩*॰ ( ¨̮ ) ॰*✩ Terima kasih sudah menjadi teman yuki kak ✩*॰ ( ¨̮ ) ॰*✩ ")
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
    try:
        sendMessage(" (｡•́︿•̀｡) Sampai jumpa lagi dengan yuki kak " + op.param1, client.getContact(op.param3).displayName )
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage("ヽ(●ﾟ´Д｀ﾟ●)ﾉﾟ kenapa kak " + op.param1, client.getContact(op.param2).displayName  )
    except Exception as e:
        print e
        ...
        if msg.toType == 2:
            if msg.contentType == 0:
                ...
                if "kick:" in msg.text:
                    ...
                    sendMessage(msg.to, ""+contact.displayName+"Maaf kak")
                if "nk:" in msg.text:
                    key = msg.text[3:]
                    group = client.getGroup(msg.to)
                    ...
                    if key in Names:
                        ...
                        sendMessage(msg.to, "Haaa?")
                if msg.text == "cancel":
                    group = client.getGroup(msg.to)
                    if group.invitee is None:
                        sendMessage(op.message.to, "Invite.")
                    else:
